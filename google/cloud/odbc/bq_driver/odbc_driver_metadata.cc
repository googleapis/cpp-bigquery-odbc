...
SQLRETURN SQLStatisticsInternal(SQLHSTMT stmt_handle,
                                SQLCHAR const* catalog_name,
                                SQLSMALLINT catalog_name_len,
                                SQLCHAR const* schema_name,
                                SQLSMALLINT schema_name_len,
  ...
  // returns SQL_SUCCESS with an empty result set, consistent with Simba.
  if (table_name == nullptr) {
    LOG(ERROR) << "SQLStatistics:: table_name is required (null pointer passed)";
    return LogAndReturnCode(
  ...
  auto table_schema = google::cloud::odbc_bq_driver_internal::
