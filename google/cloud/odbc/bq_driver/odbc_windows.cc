...
bool ConfigDSNInternal(HWND hwnd_parent, WORD f_request, LPCSTR lpsz_driver,
  ...
  std::string language_dialect = section.count("LanguageDialect") > 0
                          ? section.at("LanguageDialect")
                          : "";
  std::string large_dataset_name=
      section.count("LargeResultsDatasetId") > 0 ? section.at("LargeResultsDatasetId") : "";
  std::string encryption_key = section.count("EncryptionKey") > 0
                        ? section.at("EncryptionKey")
                        : "";
  std::string rows_per_block = section.count("RowsFetchedPerBlock") > 0
                        ? section.at("RowsFetchedPerBlock")
                        : "";
  std::string default_string_length = section.count("DefaultStringColumnLength") > 0
                               ? section.at("DefaultStringColumnLength")
                               : "";
  std::string temp_expiration= section.count("LargeResultsTempTableExpirationTime") > 0
                               ? section.at("LargeResultsTempTableExpirationTime")
                               : "";
  std::string session_location = section.count("SessionLocation") > 0
                          ? section.at("SessionLocation")
                          : "";
  std::string additional_projects = section.count("AdditionalProjects") > 0
                             ? section.at("AdditionalProjects")
                             : "";
  std::string query_properties = section.count("QueryProperties") > 0
                          ? section.at("QueryProperties")
                          : "";
  std::string activation_threshold= section.count(" HTAPI_ActivationThreshold") > 0
