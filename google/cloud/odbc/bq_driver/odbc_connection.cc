...
SQLRETURN SQLDriverConnectInternal(SQLHDBC conn_handle, SQLHWND window_handle,
  ...
  switch (driver_completion) {
    case SQL_DRIVER_PROMPT: {
      if (!window_handle) {
        return LogAndReturnCode(*handle_ref, StatusRecord{SQLStates::k_HY092(), 
...
break;
    ...
    case SQL_DRIVER_COMPLETE_REQUIRED: {
        ...
        return LogAndReturnCode(*handle_ref, StatusRecord{SQLStates::k_IM008(), "Dialog failed"});
      }
      break;
    }
    case SQL_DRIVER_NOPROMPT: {
      if (conn_string.empty()) {
        return LogAndReturnCode(*handle_ref, StatusRecord{SQLStates::k_IM002(),
                "Data source not found and no default driver specified"});
      }
      break;
    }
    default:
      return LogAndReturnCode(*handle_ref, StatusRecord{SQLStates::k_HY110(),
