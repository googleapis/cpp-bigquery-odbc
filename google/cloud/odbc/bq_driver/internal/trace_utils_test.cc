...
#ifdef _WIN32
Section const kWINDriverSection1{{"LogLevel", "6"},
                                 {"LogPath", "C:\\Users\\Admin\\Desktop"}};
...
TEST(TraceLoggingFile, WINTraceOptionsFromConfigTraceEnabled) {
  ...
  auto sections_status = ParseConfig(
      "SOFTWARE\\WOW6432Node\\Google\\Google ODBC Driver for Google BigQuery");
#else
  auto sections_status =
      ParseConfig("SOFTWARE\\Google\\Google ODBC Driver for Google BigQuery");
#endif  // _WIN64
  ...
  for (auto const& it_outer : kWINConfigSections1) {
    ...
    Section sample_driver_section = it_outer.second;

    for (auto& it_inner : sample_driver_section) {
