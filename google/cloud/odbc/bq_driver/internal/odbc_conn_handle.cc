...
void ConnectionHandle::SetUp(Section& dsn_section,
  ...
  LOG(INFO) << "ConnectionHandle::SetUp:: Setting up connection handle from DSN configuration for DSN: " << dsn_name;
...
StatusRecord ConnectionHandle::ValidateExternalUser(
  ...
  if (auth.oauth.auth_mechanism == OauthMechanism::kExternalUser) {
    ...
    if (auth.oauth.credentials_file_path.empty()) {
      LOG(ERROR) << "ConnectionHandle::ValidateExternalUser:: JSON Credentials File path is empty for external user.";
      return StatusRecord{
          SQLStates::k_HY000(),
...
StatusRecord ConnectionHandle::Connect(Authentication& auth) {
  ...
  if (!response) {
    LOG(ERROR) << "ConnectionHandle::Connect::CreateBQClient:: " << response.GetStatusRecord().message;
  ...
  if (!access_token_resp) {
    LOG(ERROR) << "ConnectionHandle::Connect::GetOAuth2Token:: " << access_token_resp.GetStatusRecord().message;
...
odbc_internal::StatusRecord ConnectionHandle::GetAttribute(
  ...
  if (!status_record.ok()) {
    LOG(ERROR) << "ConnectionHandle::GetAttribute::ValidateConnection:: " << status_record.message;
...
StatusRecord ConnectionHandle::SetAttribute(SQLINTEGER attribute,
  ...
  auto status_record =
      ValidateConnection(IsConnected(), err_msg,
                         conn_attr.GetAttributeConnectionBehavior(attribute));
  if (!status_record.ok()) {
    LOG(ERROR) << "ConnectionHandle::SetAttribute::ValidateConnection:: " << status_record.message;
    return status_record;
  }
...
odbc_internal::StatusRecord ConnectionHandle::ValidateBYOIDProperties(
  ...
  if ((byoid_aud_url.empty() || byoid_subj_token_type.empty() ||
    ...
    LOG(ERROR) << "ConnectionHandle::ValidateBYOIDProperties:: Required BYOID properties not set.";
    return StatusRecord{SQLStates::k_HY000(),
  ...
  if (byoid_subj_token_type != kSubTokenTypeJWT &&
    ...
    LOG(ERROR) << "ConnectionHandle::ValidateBYOIDProperties:: Invalid subject token type: " << byoid_subj_token_type;    
