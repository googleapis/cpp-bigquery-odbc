...
StatusRecord DescriptorRecord::SetNumPrecRadix(SQLINTEGER value) {
  if (value != kNumPrecRadixForNonNumeric &&
      ...
      LOG(ERROR) << "DescriptorRecord::SetNumPrecRadix:: Invalid attribute/option identifier: " << value;
...
StatusRecord DescriptorRecord::SetParameterType(SQLSMALLINT value) {
  if (value != SQL_PARAM_INPUT && value != SQL_PARAM_INPUT_OUTPUT &&
      ...
      LOG(ERROR) << "DescriptorRecord::SetParameterType:: Invalid parameter type: " << value;
...
StatusRecord DescriptorRecord::SetUnnamed(SQLSMALLINT value) {
  if (value != SQL_UNNAMED) {
    LOG(ERROR) << "DescriptorRecord::SetUnnamed:: Invalid descriptor field identifier: " << value;
...
StatusRecord DescriptorRecord::SetOtherCType(SQLSMALLINT const value,
  ...
  switch (value) {
    ...
    case SQL_C_GUID:
      ...
      break;
    default:
      LOG(ERROR) << "DescriptorRecord::SetOtherCType:: " << error_message << ": " << value;
...
StatusRecord DescriptorRecord::SetOtherSQLType(
  ...
  switch (value) {
    ...
    case SQL_GUID:
      ...
      break;
    default:
      LOG(ERROR) << "DescriptorRecord::SetOtherSQLType:: " << error_message << ": " << value;
      return StatusRecord{SQLStates::k_HY021(), error_message};
  }
...
StatusRecord DescriptorRecord::SetDisplaySize(SQLSMALLINT type,
  ...
  if (!type) {
    LOG(ERROR) << "DescriptorRecord::SetDisplaySize:: Invalid attribute/option identifier (type is null).";
...
StatusRecord DescriptorRecord::SetOctetLength(SQLSMALLINT type,
                                              ...
                                              SQLINTEGER precision) {
  if (!type) {
    LOG(ERROR) << "DescriptorRecord::SetOctetLength:: Invalid attribute/option identifier (type is null).";
    return StatusRecord{SQLStates::k_HY092(),
                        "Invalid attribute/option identifier"};
  }
  switch (type) {
...
StatusRecord DescriptorRecord::SetType(SQLSMALLINT value,
  ...
  if (value == SQL_INTERVAL) {
    for (auto const& entry : kIntervalTypes) {
     ...
     LOG(ERROR) << "DescriptorRecord::SetType:: Interval code " << datetime_interval_code << " invalid or not supported.";
  ...
  if (value == SQL_DATETIME) {
    ...
    LOG(ERROR) << "DescriptorRecord::SetType:: Datetime interval code " << datetime_interval_code << " invalid or not supported.";
    return StatusRecord{SQLStates::k_HY021(),
...
StatusRecord DescriptorRecord::ConsistencyCheck() const {
  ...
  LOG(ERROR) << "DescriptorRecord::ConsistencyCheck:: Inconsistent descriptor information. Type: " << type << ", ConciseType: " << concise_type;
  return StatusRecord{SQLStates::k_HY021(),
...
StatusRecord DescriptorRecord::SetDataPointer(SQLPOINTER ptr,
  ...
  if (!status_record.ok()) {
     LOG(ERROR) << "DescriptorRecord::SetDataPointer::ConsistencyCheck:: " << status_record.message;
