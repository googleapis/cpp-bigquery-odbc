...
StatusRecord PopulateOutputConnectionString(SQLCHAR* out_conn_str,
  ...
  std::cout << "[DEBUG] Input connection string: \"" << conn_string << "\""
            << std::endl;
  std::cout << "[DEBUG] Output buffer length: " << out_conn_str_buflen
            << std::endl;
  std::cout << "[DEBUG] is_conn_str_empty flag: "
            << (is_conn_str_empty ? "true" : "false") << std::endl;

  if (is_conn_str_empty) {
    if (conn_string.empty()) {
      std::cout << "[ERROR] Connection string is empty when it shouldn't be"
                << std::endl;
      return StatusRecord{SQLStates::k_HY000(), "Invalid Connection String"};
    }
  }
  ...
  if (!out_tmp_str.empty() && out_tmp_str.back() != ';') {
    out_tmp_str.append(";");
    std::cout << "[DEBUG] Appended semicolon to connection string: \""
              << out_tmp_str << "\"" << std::endl;
  }

  auto out_str_len = out_tmp_str.length();
  std::cout << "[DEBUG] Final output string length: " << out_str_len
            << std::endl;
  ...
  if (out_str_len >= out_conn_str_buflen) {
    ...
    if (out_conn_str_len) {
      *out_conn_str_len = out_str_len;
      std::cout << "[DEBUG] Reported length (with truncation): "
                << *out_conn_str_len << std::endl;
    }
  ...
  if (out_conn_str_len) {
    *out_conn_str_len = out_tmp_str.length();
    std::cout << "[DEBUG] Output connection string length: "
              << *out_conn_str_len << std::endl;
  }
