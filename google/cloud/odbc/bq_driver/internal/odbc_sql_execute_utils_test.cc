...
void PopulateDescriptors(DescriptorHandle& apd, DescriptorHandle& ipd, int col_ind, SQLSMALLINT c_type, SQLSMALLINT sql_type, SQLPOINTER value_ptr, SQLLEN buf_len, SQLLEN* ind_ptr) {
...
TEST(ConstructPositionalQueryParams, Basic) {
  ...
  PopulateDescriptors(apd, ipd, 1, SQL_C_FLOAT, SQL_CHAR, &float_value, 0, nullptr);

  QueryParameter int_param;
  int_param.parameter_type.type = "INT64";
  PopulateDescriptors(apd, ipd, 2, SQL_C_FLOAT, SQL_SMALLINT, &float_value, 0, nullptr);

  QueryParameter str_param;
  float_param.parameter_type.type = "STRING";
  ...
  std::vector<QueryParameter> query_params = {float_param, int_param, str_param};
  StatusRecord status_record = ConstructPositionalQueryParams(apd, ipd, query_params);
