...
static std::string m_keyFilePath;
static std::string m_oAuthMechanism;
static std::string m_dataset;
static std::string m_catalog;
...
HWND CreateLabel(HWND parent, const char* text, int x, int y, int width, int height, int id) {
    return CreateWindowEx(0, "STATIC", text, WS_VISIBLE | WS_CHILD | SS_LEFT, x, y, width, height, parent, (HMENU)id, GetModuleHandle(NULL), NULL);
}

// Helper function to create an edit box
HWND CreateEditBox(HWND parent, int x, int y, int width, int height, int id) {
    return CreateWindowEx(0, "EDIT", "", WS_VISIBLE | WS_CHILD | WS_BORDER | ES_LEFT, x, y, width, height, parent, (HMENU)id, GetModuleHandle(NULL), NULL);
}

// Helper function to create a combo box (dropdown)
HWND CreateComboBox(HWND parent, int x, int y, int width, int height, int id) {
    return CreateWindowEx(0, "COMBOBOX", NULL, WS_TABSTOP | WS_VISIBLE | WS_CHILD | CBS_DROPDOWN, x, y, width, height, parent, (HMENU)id, GetModuleHandle(NULL), NULL);
}

// Helper function to create a button
HWND CreateButton(HWND parent, const char* text, int x, int y, int width, int height, int id) {
    return CreateWindowEx(0, "BUTTON", text, WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, x, y, width, height, parent, (HMENU)id, GetModuleHandle(NULL), NULL);
}
...
void DriverForm::InitControls() {
    ...
    HWND hCatalogText = CreateLabel(m_hwnd, "Catalog (Project):", 20, 280, 110, 20, kIdcCatalogLabel);
    HWND hCatalogBox = CreateComboBox(m_hwnd, 160, 280, 230, 100, kIdcCatlogBOX);

    HWND hDatasetText = CreateLabel(m_hwnd, "Dataset:", 20, 320, 50, 20, kIdcDatasetLabel);
    HWND hDatasetBox = CreateComboBox(m_hwnd, 160, 320, 230, 100, kIdcDatasetBOX);

    HWND hwndOkButton =CreateButton(m_hwnd, "Ok", 220, 400, 80, 30, kIdcButtonOk);
    HWND hwndCancelButton =CreateButton(m_hwnd, "Cancel", 320, 400, 80, 30, kIdcButtonCancel);
...
const std::string& DriverForm::GetEmail() const {
    return m_email;
}

const std::string& DriverForm::GetKeyFilePath() const {
    return m_keyFilePath;
}

const std::string& DriverForm::GetOAuthMechanism() const {
    return m_oAuthMechanism;
}

const std::string& DriverForm::GetDatasetName() const {
    return m_dataset;
}

const std::string& DriverForm::GetCatalogName() const {
    return m_catalog;
}
