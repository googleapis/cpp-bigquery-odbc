...
void LogTraceDialog::SetValues(Section const& attributes_map) {
  if (attributes_map.count("LogLevel") > 0) {
    if (attributes_map.at("LogLevel") == "0") {
      log_level_ = "LOG_OFF";
    } else if (attributes_map.at("LogLevel") == "6") {
  ...
  log_file_path_ =
      attributes_map.count("LogFile") > 0 ? attributes_map.at("LogFile") : "";
}
...
void DriverForm::InitControls() {
  ...
  HWND h_logging_button = CreateButton(m_hwnd, "Logging Options", 200, 370, 120,
                                       30, kIdcLoggingBtn);
  ...
  SetControlFont(h_logging_button, h_font);
...
void LogTraceDialog::InitControls() {
  ...
  HWND h_log_file_add =
      CreateLabel(parent_hwnd, "Log File:", 20, 80, 80, 20, 0);
...
void LogTraceDialog::Show(HWND hwnd) {
  ...
  parent_hwnd = CreateWindowEx(0, CLASS_NAME, "Logging Options",
                               WS_OVERLAPPEDWINDOW, xPos, yPos, 450, 300, hwnd,
        ...
        case kIdclogTraceBox: {
          if (HIWORD(w_param) == CBN_SELCHANGE) {
            ...
            if (selected_index != CB_ERR) {
              ...
              // Enable/Disable controls based on the selected value
...
LRESULT CALLBACK DriverForm::WindowProc(HWND hwnd, UINT u_msg, WPARAM w_param,
  ...
  auto log_form = new LogTraceDialog;
