...
std::string DriverForm::SetValues(std::string attr) {
  ...
  while (start < attr.size()) {
    ...
    std::string field = attr.substr(start, end - start);
    size_t delimiter = field.find('=');

    if (delimiter != std::string::npos && delimiter + 1 < field.size()) {
  ...
  email = attributesMap.count("email") > 0 ? attributesMap["email"] : "";
  oAuthMechanism = attributesMap.count("oauthmechanism") > 0
                       ? attributesMap["oauthmechanism"]
                       : "";
  keyFilePath = attributesMap.count("keyfilepath") > 0
                    ? attributesMap["keyfilepath"]
                    : "";
  catalog = attributesMap.count("catalog") > 0 ? attributesMap["catalog"] : "";
  dataset = attributesMap.count("dataset") > 0 ? attributesMap["dataset"] : "";
...
void DriverForm::InitControls() {
  ...
  SetWindowText(hEmailEdit, email.c_str());
  SetWindowText(hKeyFileEdit, keyFilePath.c_str());
  SetWindowText(hCatalogBox, catalog.c_str());
  SetWindowText(hDatasetBox, dataset.c_str());
  SetWindowText(hComboBox, oAuthMechanism.c_str());
}
...
std::string const& DriverForm::GetEmail() const { return email; }

std::string const& DriverForm::GetKeyFilePath() const { return keyFilePath; }

std::string const& DriverForm::GetOAuthMechanism() const {
  return oAuthMechanism;
}
