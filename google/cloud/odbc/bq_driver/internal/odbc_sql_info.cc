...
void SQLGetInfoVal::AddDiagnostics(ConnectionHandle* handle,
  ...
  if (handle) {
    LOG(INFO) << "SQLGetInfoVal::AddDiagnostics:: Adding diagnostic record: [SQLSTATE=" << status_record.sql_state << "] " << status_record.message;
...
SQLRETURN SQLGetInfoSqlChar::InfoValToResponse(ConnectionHandle* handle,
  ...
  auto status_record = StringValueToOutputBufferResponse(
      ...
      str_len_ptr);
  if (!status_record.ok()) {
    LOG(WARNING) << "SQLGetInfoSqlChar::InfoValToResponse:: " << status_record.message;
