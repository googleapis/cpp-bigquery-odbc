...
TEST(BuildTableSchemaFromRowSchema, RowSchemaIsEmpty) {
  ...
  EXPECT_EQ(result.GetStatusRecord().message, "row schema should not be less than 0");
}
...
TEST(BuildTableSchemaFromRowSchema, Fail_ColIndexNotFound) {
  ...
  ASSERT_FALSE(result.Ok());
  EXPECT_TRUE(result.GetStatusRecord().message.find("No matching col_index found: 1") != std::string::npos);
}
...
TEST(BuildTableSchemaFromRowSchema, Success_CheckModeRepeated) {
  ...
  RowSchema row_schema = {
      ColumnSchema{0, BQDataType::kString, true}
  };
