...
StatusRecordOr<std::string> ConvertFromArithmeticValue(SrcType src_val, SQLSMALLINT sql_type) {
  ...
  switch (sql_type) {
    ...
    case SQL_REAL:
    case SQL_FLOAT: {
      StatusRecord check_status = CheckLimitsArithmetic<SrcType, SQLREAL>(src_val);
    ...
    case SQL_DOUBLE: {
      StatusRecord check_status = CheckLimitsArithmetic<SrcType, SQLDOUBLE>(src_val);
      // In case of 'Numeric value out of range'(22003), we should throw error
      if (check_status.sql_state != SQLStates::k_22003()) {
        // We need to typecast it to dest type once
    ...
    case SQL_BIGINT: {
      StatusRecord check_status = CheckLimitsArithmetic<SrcType, SQLBIGINT>(src_val);
      // In case of 'Numeric value out of range'(22003), we should throw error
      if (check_status.sql_state != SQLStates::k_22003()) {
        // We need to typecast it to dest type once
    ...
    case SQL_SMALLINT: {
      StatusRecord check_status = CheckLimitsArithmetic<SrcType, SQLSMALLINT>(src_val);
      // In case of 'Numeric value out of range'(22003), we should throw error
      if (check_status.sql_state != SQLStates::k_22003()) {
        // We need to typecast it to dest type once
    ...
    case SQL_TINYINT: {
      StatusRecord check_status = CheckLimitsArithmetic<SrcType, SQLCHAR>(src_val);
      // In case of 'Numeric value out of range'(22003), we should throw error
      if (check_status.sql_state != SQLStates::k_22003()) {
        // We need to typecast it to dest type once
    ...
    case SQL_INTEGER: {
      StatusRecord check_status = CheckLimitsArithmetic<SrcType, SQLINTEGER>(src_val);
      // In case of 'Numeric value out of range'(22003), we should throw error
      if (check_status.sql_state != SQLStates::k_22003()) {
        // We need to typecast it to dest type once
...
// Translates src_data which is for a C Data Type -> SQL Data Type -> std::string
// This function is typically used to translate SQL_PARAM_INPUT type of parameter
// binding.
