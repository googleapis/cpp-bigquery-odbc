...
StatusRecordOr<Procedure> ValidateProcedureColumnParameters(
  ...
  if (procedure_name_len < 0 && procedure_name_len != SQL_NTS) {
      LOG(ERROR) << "ValidateProcedureColumnParameters:: Invalid procedure name length.";
    return StatusRecord{SQLStates::k_HY090(), "Invalid procedure name length"};
  }

  if (metadata_id == SQL_TRUE) {
    if (!catalog_name) {
      LOG(ERROR) << "ValidateProcedureColumnParameters:: Catalog name cannot be NULL.";
      return StatusRecord{SQLStates::k_HY009(), "Catalog name cannot be NULL"};
    }
    if (!schema_name) {
      LOG(ERROR) << "ValidateProcedureColumnParameters:: Schema name cannot be NULL.";
      return StatusRecord{SQLStates::k_HY009(), "Schema name cannot be NULL"};
    }
    if (!procedure_name) {
      LOG(ERROR) << "ValidateProcedureColumnParameters:: Procedure name cannot be NULL.";
  ...
  if (IsSearchPatternArgument(reinterpret_cast<char const*>(catalog_name))) {
    LOG(ERROR) << "ValidateProcedureColumnParameters:: Catalog name cannot be a search pattern.";
  ...
  if (catalog.empty()) {
    LOG(ERROR) << "ValidateProcedureColumnParameters:: Catalog cannot be empty.";
    return StatusRecord{SQLStates::k_HY000(), "Catalog cannot be empty"};
  }
  if (dataset.empty()) {
    LOG(ERROR) << "ValidateProcedureColumnParameters:: Dataset cannot be empty.";
    return StatusRecord{SQLStates::k_HY000(), "Dataset cannot be empty"};
  }
  if (proc_name.empty()) {
    LOG(ERROR) << "ValidateProcedureColumnParameters:: Procedure name cannot be empty.";
...
StatusRecordOr<Procedure> FetchBQProcedureData(ConnectionHandle& conn_handle,
  ...
  if (!conn_handle.IsConnected()) {
    LOG(ERROR) << "FetchBQProcedureData:: Connection to the data source is broken.";
  ...
  if (!bq_client) {
    LOG(ERROR) << "FetchBQProcedureData:: Invalid or null BQ Client within the connection handle.";
    return StatusRecord{
        SQLStates::k_HY000(),
  ...
  if (!query_result.Ok()) {
    LOG(ERROR) << "FetchBQProcedureData::Query:: Failed to fetch procedure data: ";
  ...
  for (auto const& row : response.rows) {
    ...
    if (columns.size() < 8) {
      LOG(ERROR) << "FetchBQProcedureData:: Unexpected column count in the response.";
...
StatusRecordOr<std::vector<FilteredProcedureResponse>> GetFilteredProcedures(
  ...
  if (!post_query_request_status) {
     LOG(ERROR) << "GetFilteredProcedures::ConstructNamedParametersPostQueryRequest:: " << post_query_request_status.GetStatusRecord().message;
  ...
  if (!fetch_status_record_or) {
    LOG(ERROR) << "GetFilteredProcedures::FetchBQData:: " << fetch_status_record_or.GetStatusRecord().message;
  ...
  if (!rows) {
    LOG(ERROR) << "GetFilteredProcedures::GetRowsResults:: " << rows.GetStatusRecord().message;
...
StatusRecordOr<SQLProcedures> FetchBQSQLProcedureData(
  ...
  if (!conn_handle.IsConnected()) {
  LOG(ERROR) << "FetchBQSQLProcedureData:: Connection to the data source is broken.";
    return StatusRecord{SQLStates::k_08S01(),
                        "Connection to the data source is broken"};
  }
  auto bq_client = conn_handle.GetClient();
  ...
  if (!query_result2.Ok()) {
    LOG(ERROR) << "FetchBQSQLProcedureData:: Failed to fetch procedure parameter data.";
    return StatusRecord{SQLStates::k_HY000(),
  ...
  if (columns.size() < 4) {
    LOG(ERROR) << "FetchBQSQLProcedureData:: Unexpected column count in procedure response.";
    return StatusRecord{SQLStates::k_HY000(),
  ...
  if (param_columns.size() < 2) {
    LOG(ERROR) << "FetchBQSQLProcedureData:: Unexpected column count in parameter response.";
    return StatusRecord{SQLStates::k_HY000(),
...
StatusRecordOr<std::vector<ProcedureType>> FetchProceduresData(
  ...
  if (!conn_handle.IsConnected()) {
    LOG(ERROR) << "FetchProceduresData:: Connection to the data source is broken.";
    return StatusRecord{SQLStates::k_08S01(),
                        "Connection to the data source is broken"};
  }
  ...
  auto bq_client = conn_handle.GetClient();
  if (!bq_client) {
    LOG(ERROR) << "FetchProceduresData:: Invalid or null BQ Client within the connection handle.";
    return StatusRecord{
        SQLStates::k_HY000(),
        "Invalid or null BQ Client within the connection handle"};
  }
