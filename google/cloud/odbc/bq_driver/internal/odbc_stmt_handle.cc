...
StatusRecord StatementHandle::SetDescriptorHandle(
  ...
  switch (type) {
    ...
    case DescriptorType::kAPD:
      ...
      break;
    default:
      LOG(ERROR) << "StatementHandle::SetDescriptorHandle:: Invalid attempt to set an implementation descriptor.";
...
StatusRecord StatementHandle::SetAttribute(int attribute, SQLULEN value) {
  ...
  if (!status_record.ok()) {
    LOG(ERROR) << "StatementHandle::SetAttribute::ValidateStatementAttributeToSet:: " << status_record.message;
...
StatusRecord StatementHandle::PopulateResultSet(TableSchema const& schema) {
  for (int i = 0; i < schema.fields.size(); ++i) {
    ...
    if (!type_status_record.Ok()) {
      LOG(ERROR) << "StatementHandle::PopulateResultSet::ConvertDSType:: " << type_status_record.GetStatusRecord().message;
...
StatusRecord StatementHandle::PrepareQuery(std::string const& query) {
  ...
  if (!transaction_status.ok()) {
    LOG(ERROR) << "StatementHandle::PrepareQuery::BeginTransactionIfNeeded:: " << transaction_status.message;
  ...
  if (!response.Ok()) {
    LOG(ERROR) << "StatementHandle::PrepareQuery::InsertJob:: " << response.GetStatusRecord().message;
  ...
  if (!pop_response.ok()) {
    LOG(ERROR) << "StatementHandle::PrepareQuery::PopulateResultSet:: " << pop_response.message;
    return pop_response;
  }
  ...
  if (!ird_response.ok()) {
    LOG(ERROR) << "StatementHandle::PrepareQuery::PopulateIrd:: " << ird_response.message;
    return ird_response;
  }
  ...
  if (!ipd_response.ok()) {
    LOG(ERROR) << "StatementHandle::PrepareQuery::PopulateIpd:: " << ipd_response.message;
    return ipd_response;
  }
  if (!conn_handle.IsSessionStarted() &&
...
StatusRecord StatementHandle::PopulateIrd(DescriptorHandle& descriptor_handle,
  ...
  for (int i = 0; i < schema.fields.size(); ++i) {
    ...
    if (!type_status_record.Ok()) {
      LOG(ERROR) << "StatementHandle::PopulateIrd::GetSQLDataType:: " << type_status_record.GetStatusRecord().message;
      return type_status_record.GetStatusRecord();
    }
    StatusRecord status_record = descriptor_record.SetConciseType(
        *type_status_record, DescriptorType::kIRD);
    if (!status_record.ok()) {
      LOG(ERROR) << "StatementHandle::PopulateIrd::SetConciseType:: " << status_record.message;
      return status_record;
    }
...
StatusRecordOr<SQLULEN> StatementHandle::GetAttribute(int attribute) {
  if (!IsStatementAttributeValid(attribute)) {
    LOG(ERROR) << "StatementHandle::GetAttribute::Invalid attribute: " << attribute;
