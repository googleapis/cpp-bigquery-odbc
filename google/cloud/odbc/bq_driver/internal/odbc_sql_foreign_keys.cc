...
odbc_internal::StatusRecordOr<DSResults> FetchForeignKeysFromDataSource(
  ...
  if (catalog_name.empty() ||
      ...
      LOG(ERROR) << "FetchForeignKeysFromDataSource:: Catalog name for both primary and foreign keys cannot be empty.";
    auto status_record =
  ...
  if (!pk_catalog_name.empty() && !fk_catalog_name.empty() &&
    ...
    LOG(ERROR) << "FetchForeignKeysFromDataSource:: PK and FK catalog names need to be the same.";
    auto status_record =
  ...
  if (schema_name.empty() ||
    ...
    LOG(ERROR) << "FetchForeignKeysFromDataSource:: Schema name for both primary and foreign keys cannot be empty.";
    auto status_record =
        StatusRecord{SQLStates::k_HY090(),
  ...
  if (!pk_schema_name.empty() && !fk_schema_name.empty() &&
    ...
    LOG(ERROR) << "FetchForeignKeysFromDataSource:: PK and FK schema names need to be the same.";
    auto status_record =
        StatusRecord{SQLStates::k_HYC00(),
                     "Optional feature not supported by the data source: PK "
  ...
  if ((pk_table_name.empty() && fk_table_name.empty()) ||
    ...
    LOG(ERROR) << "FetchForeignKeysFromDataSource:: Both Primary and Foreign key table names cannot be empty.";
  ...
  if (!query_param_status) {
    LOG(ERROR) << "FetchForeignKeysFromDataSource::ConstructStringQueryParameters:: " << query_param_status.GetStatusRecord().message;
  ...
  if (!post_query_request_status) {
    LOG(ERROR) << "FetchForeignKeysFromDataSource::ConstructNamedParametersPostQueryRequest:: " << post_query_request_status.GetStatusRecord().message;
  ...
  if (!status_record_or) {
    LOG(ERROR) << "FetchForeignKeysFromDataSource::FetchBQData:: " << status_record_or.GetStatusRecord().message;
