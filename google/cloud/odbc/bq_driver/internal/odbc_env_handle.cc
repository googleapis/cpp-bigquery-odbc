  ...
  switch (attribute) {
    case SQL_ATTR_CONNECTION_POOLING: {
      if (connection_pool_ == nullptr) {
        LOG(ERROR) << "EnvironmentHandle::GetAttribute:: Internal error: null connection pool";
    ...
    case SQL_ATTR_CP_MATCH: {
      if (connection_pool_match_ == nullptr) {
        LOG(ERROR) << "EnvironmentHandle::GetAttribute:: Internal error: cp_match not initialized";
        auto status_record = StatusRecord{
            SQLStates::k_HY001(),
    ...
    case SQL_ATTR_ODBC_VERSION: {
      if (odbc_ver_ == nullptr) {
        LOG(ERROR) << "EnvironmentHandle::GetAttribute:: Internal error: odbc_version not initialized";
        auto status_record =
    ...
    case SQL_ATTR_OUTPUT_NTS: {
      if (output_nts_ == nullptr) {
        LOG(ERROR) << "EnvironmentHandle::GetAttribute:: Internal error: output_nts not initialized";
        auto status_record =
            StatusRecord{SQLStates::k_HY001(),
...
SQLRETURN EnvironmentHandle::SetAttribute(SQLINTEGER attribute, void* value,
  ...
  switch (attribute) {
    case SQL_ATTR_CONNECTION_POOLING: {
      ...
      if (!conn_pool_val) {
        LOG(ERROR) << "EnvironmentHandle::SetAttribute::ParseVal:: " << conn_pool_val.GetStatusRecord().message;
    ...
    case SQL_ATTR_CP_MATCH: {
      ...
      if (!cp_match_val) {
        LOG(ERROR) << "EnvironmentHandle::SetAttribute::ParseVal:: " << cp_match_val.GetStatusRecord().message;
    ...
    case SQL_ATTR_ODBC_VERSION: {
      ...
      if (!odbc_vers_val) {
        LOG(ERROR) << "EnvironmentHandle::SetAttribute::ParseVal:: " << odbc_vers_val.GetStatusRecord().message;
    ...
    case SQL_ATTR_OUTPUT_NTS: {
      ...
      if (!output_nts_val) {
        LOG(ERROR) << "EnvironmentHandle::SetAttribute::ParseVal:: " << output_nts_val.GetStatusRecord().message;
