...
StatusRecord CreateResultSetRowSchema(ResultSet& result_set) {
  for (auto const& entry : kODBCColumnsMap) {
    ...
    if (!col_schema_status) {
      LOG(ERROR) << "CreateResultSetRowSchema::GetColumnSchema:: " << col_schema_status.GetStatusRecord().message;
...
StatusRecordOr<DSRow> CreateResultSetDSRow(ConnectionHandle const& conn_handle,
  ...
  if (!data_type_status) {
    LOG(ERROR) << "CreateResultSetDSRow::GetSQLDataType:: " << data_type_status.GetStatusRecord().message;
  ...
  if (!type_status) {
    LOG(ERROR) << "CreateResultSetDSRow::GetTypeDescription:: " << type_status.GetStatusRecord().message;
  ...
  if (!col_size_status) {
    LOG(ERROR) << "CreateResultSetDSRow::GetColSize:: " << col_size_status.GetStatusRecord().message;
  ...
  if (!buf_len_status) {
    LOG(ERROR) << "CreateResultSetDSRow::GetBufferLen:: " << buf_len_status.GetStatusRecord().message;
  ...
  if (!dec_digits_status) {
    LOG(ERROR) << "CreateResultSetDSRow::GetDecimalDigits:: " << dec_digits_status.GetStatusRecord().message;
  ...
  if (!radix_status) {
    LOG(ERROR) << "CreateResultSetDSRow::GetRadix:: " << radix_status.GetStatusRecord().message;
  ...
  if (field_pos < 0) {
    LOG(ERROR) << "CreateResultSetDSRow:: Invalid ordinal position: " << field_pos;
...
StatusRecordOr<Table> FetchBQTableData(ConnectionHandle& conn_handle,
  ...
  if (catalog.empty()) {
    LOG(ERROR) << "FetchBQTableData:: Catalog cannot be empty for BQ Data source.";
    return StatusRecord{SQLStates::k_HY000(),
                        "Catalog cannot be empty for BQ Data source"};
  }
  if (dataset.empty()) {
    LOG(ERROR) << "FetchBQTableData:: Dataset cannot be empty for BQ Data source.";
    return StatusRecord{SQLStates::k_HY000(),
                        "Dataset cannot be empty for BQ Data source"};
  }
  if (table.empty()) {
    LOG(ERROR) << "FetchBQTableData:: Table cannot be empty for BQ Data source.";
    return StatusRecord{SQLStates::k_HY000(),
  ...
  if (!bq_client) {
    LOG(ERROR) << "FetchBQTableData:: Invalid or null BQ Client within the connection handle.";
    return StatusRecord{
        SQLStates::k_HY000(),
...
StatusRecordOr<ResultSet> ProcessTableResults(
  ...
  if (!metadata_id && (bq_table_column.empty() || bq_table_column == "%")) {
    ...
    for (TableFieldSchema const& table_field_schema : bq_table.schema.fields) {
      ...
      if (!ds_row_status) {
        LOG(ERROR) << "ProcessTableResults::CreateResultSetRowSchema:: " << row_schema_status.message;
    ...
    for (TableFieldSchema const& table_field_schema : bq_table.schema.fields) {
      ...
      if (std::regex_match(table_field_schema.name, column_pattern)) {
        auto ds_row_status = CreateResultSetDSRow(
            ...
            bq_table.table_reference.table_id, table_field_schema, ord_pos);
        if (!ds_row_status) {
          LOG(ERROR) << "ProcessTableResults::CreateResultSetDSRow:: " << ds_row_status.GetStatusRecord().message;
          return ds_row_status.GetStatusRecord();
        }
        result_set.rows.emplace_back(*ds_row_status);
        break;
...
StatusRecordOr<std::vector<Table>> FetchBQTablesData(
  ...
  if (catalog.empty()) {
    LOG(ERROR) << "FetchBQTablesData:: Catalog cannot be empty for BQ Data source.";
    return StatusRecord{SQLStates::k_HY000(),
                        "Catalog cannot be empty for BQ Data source"};
  }
  if (dataset_pattern.empty()) {
    LOG(ERROR) << "FetchBQTablesData:: Dataset pattern cannot be empty for BQ Data source.";
    return StatusRecord{SQLStates::k_HY000(),
                        "Dataset pattern cannot be empty for BQ Data source"};
  }
  if (table_pattern.empty()) {
    LOG(ERROR) << "FetchBQTablesData:: Table pattern cannot be empty for BQ Data source.";
    return StatusRecord{SQLStates::k_HY000(),
                        "Table pattern cannot be empty for BQ Data source"};
  }
  if (!conn_handle.IsConnected()) {
    LOG(ERROR) << "FetchBQTablesData:: Connection to the data source is broken.";
    return StatusRecord{SQLStates::k_08S01(),
                        "Connection to the data source is broken"};
  }
  auto bq_client = conn_handle.GetClient();
  if (!bq_client) {
    LOG(ERROR) << "FetchBQTablesData:: Invalid or null BQ Client within the connection handle.";
    return StatusRecord{
        SQLStates::k_HY000(),
        "Invalid or null BQ Client within the connection handle"};
  }
  ...
  if (!datasets_status) {
    LOG(ERROR) << "FetchBQTablesData::GetFilteredDatasetIds:: " << datasets_status.GetStatusRecord().message;
  ...
  for (auto const& dataset : *datasets_status) {
    ...
    if (!tables_status) {
      LOG(ERROR) << "FetchBQTablesData::GetFilteredTables:: " << tables_status.GetStatusRecord().message;
    ...
    for (auto const& filtered_table : *tables_status) {
      ...
      if (!bq_table_status) {
        LOG(ERROR) << "FetchBQTablesData::FetchBQTableData:: " << bq_table_status.GetStatusRecord().message;
