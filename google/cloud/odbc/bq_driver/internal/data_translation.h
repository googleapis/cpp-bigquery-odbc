...
inline odbc_internal::StatusRecord CheckLimitsArithmetic(SrcType value) {
  ...
  if (!std::is_arithmetic_v<SrcType> || !std::is_arithmetic_v<DestType>) {
    LOG(ERROR) << "CheckLimitsArithmetic::Invalid datatypes for conversion check!";
...
inline odbc_internal::StatusRecord ConvertFromArithmeticDSValue(
  ...
  if (!std::is_arithmetic_v<SrcType>) {
    LOG(ERROR) << "ConvertFromArithmeticDSValue::Invalid datatypes for conversion check!";
  ...
  switch (dest_type) {
    ...
    case SQL_C_CHAR: {
      ...
      if (status_record.sql_state == SQLStates::k_01004()) {
        LOG(ERROR) << "ConvertFromArithmeticDSValue::StringValueToOutputBufferResponse:: Numeric value out of range";
        return StatusRecord{SQLStates::k_22003(), "Numeric value out of range"};
      }
      return status_record;
    }
    case SQL_C_BIT: {
      ...
      LOG(ERROR) << "ConvertFromArithmeticDSValue::Numeric value out of range for BIT type";
      return StatusRecord{SQLStates::k_22003(), "Numeric value out of range"};
    }
    case SQL_C_SHORT: {
      auto* dest_val = reinterpret_cast<SQLSMALLINT*>(dest_buf);
    ...
    case SQL_WCHAR: {
      ...
      if (status_record.sql_state == SQLStates::k_01004()) {
        LOG(ERROR) << "ConvertFromArithmeticDSValue::WStrToOutputBufferResponse:: Numeric value out of range";
        return StatusRecord{SQLStates::k_22003(), "Numeric value out of range"};
      }
      return status_record;
    }
    case SQL_C_NUMERIC: {
      ...
      if (abs_val > 0) {
        LOG(ERROR) << "ConvertFromArithmeticDSValue::Numeric value out of range for NUMERIC type";
        return StatusRecord{SQLStates::k_22003(), "Numeric value out of range"};
      }
      if (res_len) {
    ...
    default: {
      LOG(WARNING) << "ConvertFromArithmeticDSValue::Conversion is unsupported for C-type: " << dest_type;
...
inline odbc_internal::StatusRecordOr<SQLDOUBLE> ConvertToDouble(
  ...
  if (!std::isfinite(result)) {
    LOG(ERROR) << "ConvertToDouble::Value is NaN or infinity for string: " << str;
