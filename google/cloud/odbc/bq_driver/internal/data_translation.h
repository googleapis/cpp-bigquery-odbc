...
inline odbc_internal::StatusRecord ConvertFromIntervalDSValue(
  ...
  switch (dest_type) {
    ...
    case SQL_C_STINYINT: {
      ...
      SQLUINTEGER value;
      GetSinglePrecisionInterval(conn_interval, value, res_len);
      if (value <=
    ...
    case SQL_C_SLONG: {
      auto* dest = reinterpret_cast<SQLINTEGER*>(dest_buf);
      SQLUINTEGER value;
      GetSinglePrecisionInterval(conn_interval, value, res_len);
      if (value <=
          static_cast<SQLUINTEGER>(std::numeric_limits<SQLINTEGER>::max())) {
        *dest = static_cast<SQLINTEGER>(value);
        if (res_len) {
