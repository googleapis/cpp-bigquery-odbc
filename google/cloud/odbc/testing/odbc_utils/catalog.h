...
class Catalog {
 public:
  ...
  static std::vector<SQLColumnsResult> GetColumns(
      const std::shared_ptr<ODBCHandles>& conn, std::string const& project_id = "",
      char const* dataset = nullptr, char const* table = nullptr,
      char const* column = nullptr, bool use_ansi = false);

  // Uses the SQLPrimaryKeys API to fetch primary keys in a dataset.
   static RowWiseResults GetPrimaryKeys(const std::shared_ptr<ODBCHandles>& conn,
                                       const std::string& dataset = "",
                                       const std::string& table = "",
                                       bool use_ansi = false);

  // Uses the SQLForeignKeys API to fetch foreign keys in a dataset.
  static RowWiseResults GetForeignKeys(const std::shared_ptr<ODBCHandles>& conn,
                                       const std::string& dataset = "",
