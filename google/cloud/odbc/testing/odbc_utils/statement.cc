...
std::shared_ptr<Results> FetchDirect(std::shared_ptr<ODBCHandles> conn,
  ...
  Results results;
  std::vector<std::unique_ptr<SQLCHAR[]>> col_data_buffers; // Ensure buffers stay in scope
  ...
  for (int i = 0; i < num_cols; i++) {
    ...
    col_data_buffers.push_back(std::make_unique<SQLCHAR[]>(col_ptr->data_size + 1));
    col_ptr->data = col_data_buffers.back().get(); // Ensure the buffer stays in scope
