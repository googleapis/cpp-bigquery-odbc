...
TEST(ListAllProjects, ListZeroProjects) {
  ...
  EXPECT_CALL(*mock, ListProjects)
      .WillOnce([](ListProjectsRequest const&) {
          return mocks::MakeStreamRange<Project>({});
      });
  ProjectClient mocked_project_client(std::move(mock));

  StatusOr<std::vector<Project>> projects = ListAllProjects(mocked_project_client, options);
...
TEST(ListAllProjects, ListProjectsFailure) {
  ...
  EXPECT_THAT(projects, StatusIs(StatusCode::kUnauthenticated, StrEq("denied")));
}
...
TEST(GetProjects, GetOneProject) {
  ...
  StatusOr<Project> project = GetProject(mocked_project_client, expected.id, options);
...
TEST(GetProjects, ProjectNotFound) {
  ...
  StatusOr<Project> project = GetProject(mocked_project_client, "unknown-id", options);
