...
TEST(DriverInfoTest, SQLGetConnectAttrAfterConn) {
  auto conn = std::make_shared<ConnectionHandle>();
  EXPECT_EQ(Connect(kDefaultConnectionString, conn), SQL_SUCCESS);
  EXPECT_EQ(GetConnectAttribute(conn), SQL_SUCCESS);
  EXPECT_EQ(Disconnect(conn), SQL_SUCCESS);
}

TEST(DriverInfoTest, SQLGetConnectAttrBeforeConn) {
  auto conn = std::make_shared<ConnectionHandle>();
  EXPECT_EQ(GetConnectAttributeBeforeConnection(conn), SQL_SUCCESS);
}
