...
TEST(CatalogTest, ReplicatePowerBIDatasetEnumeration) {
  ...
  std::vector<SQLTableResult> results = Catalog::GetTables(
      conn, kCatalogName, SQL_ALL_SCHEMAS, /*table=*/nullptr);
  ...
  for (auto const& result : results) {
    if (result.dataset_name.has_value() &&
        result.dataset_name.value() == "kirltest") {
      found_kirltest = true;
      break;
    }
  }
  ASSERT_TRUE(found_kirltest)
      << "Expected to find the 'kirltest' dataset in the results.";
  std::cout << "Found " << results.size() << " total datasets.\n";

  EXPECT_EQ(Disconnect(conn), SQL_SUCCESS);
}

TEST(CatalogTest, SQLTables_SpecificCatalogAndDataset) {
  auto conn = std::make_shared<ODBCHandles>();
  EXPECT_EQ(Connect(kDefaultConnectionString, conn), SQL_SUCCESS);
  ...
  SQLRETURN rc = SQLTablesA(
      conn->hstmt,
      (SQLCHAR*)"bigquery-devtools-drivers", SQL_NTS,   // Catalog
      (SQLCHAR*)"%", SQL_NTS,           // Schema (dataset)
      (SQLCHAR*)"%", SQL_NTS,                           // Table name wildcard
      (SQLCHAR*)"%", SQL_NTS                            // Table type wildcard
  );
