...
TEST(SQLProcedures, RespectsMaxRows) {
  auto conn = std::make_shared<ODBCHandles>();

  // Create two procedures with different names
  std::string proc1_name = kDatasetWithTablePrefix + "MAXROWS_PROC1";
  std::string proc2_name = kDatasetWithTablePrefix + "MAXROWS_PROC2";

  std::string create_proc1 = "CREATE OR REPLACE PROCEDURE " + proc1_name +
                             "(OUT param1 INT64) BEGIN SET param1 = 1; END;";
  std::string create_proc2 = "CREATE OR REPLACE PROCEDURE " + proc2_name +
                             "(OUT param1 INT64) BEGIN SET param1 = 2; END;";

  CreateProcedure(conn, proc1_name, create_proc1);
  CreateProcedure(conn, proc2_name, create_proc2);

  // Connect and set MAX_ROWS to 1
  ...
  // Run SQLProcedures for the dataset to match all procedures
  SQLRETURN ret =
      SQLProcedures(conn->hstmt, NULL, 0,                     // Catalog
                    (SQLCHAR*)kDatasetName.c_str(), SQL_NTS,  // Schema
                    (SQLCHAR*)(kTableNamePrefix + "MAXROWS_PROC%").c_str(),
                    SQL_NTS);  // ProcedureName = all
  ASSERT_EQ(ret, SQL_SUCCESS);

  // Validate only one result is returned
  int row_count = 0;
  ...
  EXPECT_EQ(SQLCloseCursor(conn->hstmt), SQL_SUCCESS);
  EXPECT_EQ(Disconnect(conn), SQL_SUCCESS);

  // Cleanup both procedures
  CleanupRoutine(conn, "DROP PROCEDURE " + proc1_name);
  CleanupRoutine(conn, "DROP PROCEDURE " + proc2_name);
}

TEST(SQLProcedureColumns, RespectsMaxRows) {
  auto conn = std::make_shared<ODBCHandles>();

  std::string procedure_name = kDatasetWithTablePrefix + "MAXROWS_PROC_COLUMNS";
  std::string create_procedure_query =
      "CREATE OR REPLACE PROCEDURE " + procedure_name +
      "(IN param1 INT64, OUT param2 STRING, INOUT param3 BOOL) "
      "BEGIN SET param2 = 'done'; END;";

  // Create procedure with 3 parameters
  CreateProcedure(conn, procedure_name, create_procedure_query);
  ...
  // Call SQLProcedureColumns for this procedure
  SQLRETURN ret = SQLProcedureColumns(
      conn->hstmt, NULL, 0,                     // Catalog
      (SQLCHAR*)kDatasetName.c_str(), SQL_NTS,  // Schema
      (SQLCHAR*)(kTableNamePrefix + "MAXROWS_PROC_COLUMNS").c_str(),
      SQL_NTS,   // Proc name
      NULL, 0);  // Column name = all
  ASSERT_EQ(ret, SQL_SUCCESS);

  // Validate that only 1 row is returned
  int row_count = 0;
  while (SQLFetch(conn->hstmt) == SQL_SUCCESS) {
    row_count++;
  }
  ...
  EXPECT_EQ(row_count, 1)
      << "SQL_ATTR_MAX_ROWS=1 should limit SQLProcedureColumns to 1 row";
...
TEST(SQLColumns, RespectsMaxRows) {
  auto conn = std::make_shared<ODBCHandles>();

  std::string table_name = kDatasetWithTablePrefix + "MAXROWS_COL_TEST";
  std::string create_table_query = "CREATE OR REPLACE TABLE " + table_name +
                                   " (col1 INT64, col2 STRING, col3 BOOL)";

  // Create table with 3 columns
  ASSERT_EQ(Connect(kDefaultConnectionString, conn), SQL_SUCCESS);
  ExecuteSQLQuery(conn, create_table_query);
  ASSERT_EQ(SQLDisconnect(conn->hdbc), SQL_SUCCESS);

  // Reconnect and set MAX_ROWS to 1
  ASSERT_EQ(Connect(kDefaultConnectionString, conn), SQL_SUCCESS);
  ASSERT_EQ(SQLSetStmtAttr(conn->hstmt, SQL_ATTR_MAX_ROWS, (SQLPOINTER)1, 0),
            SQL_SUCCESS);

  // Call SQLColumns for this table
  SQLRETURN ret =
      SQLColumns(conn->hstmt, NULL, 0,                     // Catalog
                 (SQLCHAR*)kDatasetName.c_str(), SQL_NTS,  // Schema
                 (SQLCHAR*)(kTableNamePrefix + "MAXROWS_COL_TEST").c_str(),
                 SQL_NTS,   // Table name
                 NULL, 0);  // Column name = all
  ASSERT_EQ(ret, SQL_SUCCESS);
  ...
  EXPECT_EQ(row_count, 1)
      << "SQL_ATTR_MAX_ROWS=1 should limit SQLColumns to 1 row";

  EXPECT_EQ(SQLCloseCursor(conn->hstmt), SQL_SUCCESS);
  EXPECT_EQ(Disconnect(conn), SQL_SUCCESS);

  // Cleanup
  CleanupRoutine(conn, "DROP TABLE " + table_name);
}

TEST(SQLTables, RespectsMaxRows) {
  auto conn = std::make_shared<ODBCHandles>();
  EXPECT_EQ(Connect(kDefaultConnectionString, conn), SQL_SUCCESS);
  ...
  CheckError(status, "SQLSetStmtAttr for MAX_ROWS", conn);

  // Assuming Catalog::GetTables supports max_rows argument, add it here:
  std::vector<SQLTableResult> results =
      Catalog::GetTables(conn, "", "", "", SQL_ALL_TABLE_TYPES);
  ...
  // Check size does not exceed max_rows
