...
TEST(CatalogTest, SQLStatistics_ValidTableRowsPrints) {
  auto conn = std::make_shared<ODBCHandles>();
  std::string table_name = kDatasetWithTablePrefix + "ODBC_SQLSTATISTICS_TEST";
  ...
  auto status = SQLStatistics(
      ...
      SQL_NTS,  // Table
      SQL_INDEX_ALL, SQL_QUICK);
      std::cout << "SQLStatistics status: " << status << std::endl;
  CheckError(status, "SQLStatistics", conn);
  ASSERT_TRUE(status == SQL_SUCCESS || status == SQL_SUCCESS_WITH_INFO);
  ...
  SQLSMALLINT column_count = 0;
  status = SQLNumResultCols(conn->hstmt, &column_count);
  std::cout << "SQLStatistics column count: " << column_count << std::endl;
  CheckError(status, "SQLNumResultCols", conn);
  EXPECT_EQ(SQL_SUCCESS, status);
  EXPECT_EQ(13, column_count);
  ...
  while (true) {
    ...
    // Print values returned by SQLStatistics.
    std::cout << "SQLStatistics Row " << row_count + 1 << ": ";
    for (SQLUSMALLINT column = 1; column <= column_count; ++column) {
      SQLCHAR value[1024] = {};
      SQLLEN indicator = 0;

      status = SQLGetData(conn->hstmt, column, SQL_C_CHAR, value,
                          sizeof(value), &indicator);

      ASSERT_TRUE(status == SQL_SUCCESS ||
  ...
  std::cout << "SQLStatistics rows returned: " << row_count << std::endl;
  EXPECT_GE(row_count, 0);
...
TEST(CatalogTest, SQLStatistics_ClusteredTable) {
  auto conn = std::make_shared<ODBCHandles>();
  std::string table_name =
      kDatasetWithTablePrefix + "ODBC_SQLSTATISTICS_CLUSTERED_TEST";
  ...
  table.CreateWithPrepare(
      conn,
      "(StringField STRING, IntField INT64) CLUSTER BY StringField");
  EXPECT_EQ(Disconnect(conn), SQL_SUCCESS);
