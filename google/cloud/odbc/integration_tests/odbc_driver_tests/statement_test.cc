...
TEST(SQLMoreResults, MaxRowsHonoredInScriptWithSQLMoreResults) {
  auto conn = std::make_shared<ODBCHandles>();
  ASSERT_EQ(Connect(kDefaultConnectionString, conn), SQL_SUCCESS);

  std::string table_name = kDatasetWithTablePrefix + "MAX_ROWS_SCRIPT_TEST";
  Table table(table_name);
  ...
  ASSERT_EQ(SQLExecDirect(conn->hstmt, (SQLCHAR*)insert_stmt.c_str(), SQL_NTS),
            SQL_SUCCESS);
  ...
  // Prepare a script with two SELECT statements
  std::string script = "SELECT * FROM " + table_name +
                       " WHERE Age = 25; "
                       "SELECT * FROM " +
                       table_name + " WHERE Age = 30;";

  ASSERT_EQ(SQLExecDirect(conn->hstmt, (SQLCHAR*)script.c_str(), SQL_NTS),
            SQL_SUCCESS);
  ...
  do {
    ASSERT_EQ(SQLNumResultCols(conn->hstmt, &num_cols), SQL_SUCCESS);
    EXPECT_EQ(num_cols, 2);  // Expect two columns: Name, Age

    char name[100] = {0};
    int64_t age = 0;
    SQLLEN name_ind = 0, age_ind = 0;

    ASSERT_EQ(
        SQLBindCol(conn->hstmt, 1, SQL_C_CHAR, name, sizeof(name), &name_ind),
        SQL_SUCCESS);
    ASSERT_EQ(SQLBindCol(conn->hstmt, 2, SQL_C_SBIGINT, &age, 0, &age_ind),
              SQL_SUCCESS);

    int row_count_in_set = 0;
    while (SQLFetch(conn->hstmt) == SQL_SUCCESS) {
      ++row_count_in_set;
    }

    // Since max rows is set to 1, each result set should return only 1 row
    EXPECT_EQ(row_count_in_set, 1)
        << "Expected 1 row in result set " << result_set_index;
...
TEST(SQLGetTypeInfo, MaxRowsIsHonored) {
  auto conn = std::make_shared<ODBCHandles>();
  EXPECT_EQ(Connect(kDefaultConnectionString, conn), SQL_SUCCESS);

  // Set max rows to 2
  EXPECT_EQ(SQLSetStmtAttr(conn->hstmt, SQL_ATTR_MAX_ROWS, (SQLPOINTER)2, 0),
            SQL_SUCCESS);

  // Call SQLGetTypeInfo for all types
  ASSERT_EQ(SQLGetTypeInfo(conn->hstmt, SQL_ALL_TYPES), SQL_SUCCESS);
  ...
  // Should return no more than 2 rows
