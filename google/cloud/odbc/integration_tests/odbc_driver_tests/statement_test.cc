...
TEST(SQLMoreResults, GetDataTest) {
  ...
  // Insert test data
  auto insert_query = "INSERT INTO " + table_name + " (StringField, IntegerField, FloatField) VALUES ('TestString', 42, 3.14)";
  CheckError(SQLPrepare(conn->hstmt, (SQLCHAR*)insert_query.c_str(), insert_query.size()), "SQLPrepare", conn);
  EXPECT_EQ(SQLExecute(conn->hstmt), SQL_SUCCESS);

  // Prepare and execute select query
  auto select_query = "SELECT StringField, IntegerField, FloatField FROM " + table_name;
  CheckError(SQLPrepare(conn->hstmt, (SQLCHAR*)select_query.c_str(), select_query.size()), "SQLPrepare", conn);
  EXPECT_EQ(SQLExecute(conn->hstmt), SQL_SUCCESS);
  ...
  EXPECT_EQ(SQLGetData(conn->hstmt, 1, SQL_C_CHAR, string_data, sizeof(string_data), &string_len), SQL_SUCCESS);
  std::cout << "Fetched StringField: " << string_data << std::endl;
  EXPECT_STREQ((char*)string_data, "TestString");

  EXPECT_EQ(SQLGetData(conn->hstmt, 2, SQL_C_LONG, &int_data, 0, &int_len), SQL_SUCCESS);
  std::cout << "Fetched IntegerField: " << int_data << std::endl;
  EXPECT_EQ(int_data, 42);

  EXPECT_EQ(SQLGetData(conn->hstmt, 3, SQL_C_DOUBLE, &float_data, 0, &float_len), SQL_SUCCESS);
