...
SQLWCHAR* const kSqlFKTablew =
    reinterpret_cast<SQLWCHAR*>(const_cast<char*>(kFKTable.c_str()));
...
TEST(CatalogDemoTest, SQLForeignKeysW) {
  int res_cols = 11;
  ...
  status = SQLForeignKeysW(
      conn->hstmt, kSqlCatalogw, kSqlCatalogLen, kSqlDatasetw, kSqlDatasetLen,
      kSqlPKTablew, kSqlPKTableLen, kSqlCatalogw, kSqlCatalogLen, kSqlDatasetw,
      kSqlDatasetLen, kSqlFKTablew, kSqlFKTableLen);
  CheckError(status, "SQLForeignKeys", conn);
  std::cout << "Successfully fetched foreign Keys for Catalog: " << kCatalog
            << ", and Dataset: " << kDataset << std::endl
            << std::endl;
