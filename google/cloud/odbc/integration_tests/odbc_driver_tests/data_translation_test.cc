...
StdTimestampStringRows const kConversionFromTimestampPicosecondTestData{{
    {SQL_C_CHAR, "2024-01-20 10:20:30.123456789123", SQL_SUCCESS},

    {SQL_C_WCHAR, "2024-01-20 11:02:33.001212123456", SQL_SUCCESS},

    {SQL_C_BINARY, "2024-01-20 02:20:22.123123123451", SQL_ERROR},
}};
...
void TestTranslationsFromTimestamp(std::shared_ptr<ODBCHandles> conn,
  ...
  for (auto const& expected : kConversionFromTimestampTestData) {
    ...
    std::string expected_val =
        FormatTimeStamp(ConvertStrToTimestampStruct(expected.value));
    switch (expected.target_c_type) {
      case SQL_C_CHAR: {
        std::string returned_val = reinterpret_cast<char*>(data);
        EXPECT_EQ(returned_val, expected.value);
      ...
      case SQL_C_TYPE_TIME: {
        ...
        std::string expected_time = expected_val.substr(
            time_start, dot_pos == std::string::npos ? std::string::npos
                                                     : dot_pos - time_start);
...
// Existing driver doesnt support Picosecond
#ifdef BQ_DRIVER_INTEGRATION_TESTS
class DataTranslationTimestampPicosecondTest
    : public ::testing::TestWithParam<std::string> {};

INSTANTIATE_TEST_SUITE_P(
    TimestampOutputFormatVariants, DataTranslationTimestampPicosecondTest,
    ::testing::Values(kDefaultConnectionString +
...
TEST_P(DataTranslationTimestampPicosecondTest,
  ...
  std::string suffix =
      connection_string.find("AllowHtapiForLargeResults=1") != std::string::npos
          ? "_HTAPI"
          : "_NON_HTAPI";

  auto const table_name = kDatasetWithTablePrefix +
                          "ODBC_INSERT_TEST_TIMESTAMP_Picosecond" + suffix;
  ...
  insert_query << "INSERT INTO " << table_name << " (Id, DOB) VALUES ";
