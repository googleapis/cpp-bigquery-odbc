...
std::string FormatTimestamp(const SQL_TIMESTAMP_STRUCT& timestamp) {
  ...
  ss << timestamp.year << "-" << std::setw(2) << std::setfill('0') << timestamp.month << "-"
     ...
     << std::setw(6) << std::setfill('0') << timestamp.fraction / 1000;
  return ss.str();
}
