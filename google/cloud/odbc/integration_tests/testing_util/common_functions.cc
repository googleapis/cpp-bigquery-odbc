...
StatusOr<std::string> InsertJob(JobClient job_client) {
  ...
  if (project_id_optional->empty()) {
    return Status(StatusCode::kInvalidArgument, "CPP_BIGQUERY_ODBC_TEST_GOOGLE_CLOUD_PROJECT environment variable is not set");
  }
  if (dataset_id_optional->empty()) {
    return Status(StatusCode::kInvalidArgument, "CPP_BIGQUERY_ODBC_TEST_BIGQUERY_DATASET environment variable is not set");
  }
  if (table_name_optional->empty()) {
    return Status(StatusCode::kInvalidArgument, "CPP_BIGQUERY_ODBC_TEST_TABLE_NAME environment variable is not set");
  }
