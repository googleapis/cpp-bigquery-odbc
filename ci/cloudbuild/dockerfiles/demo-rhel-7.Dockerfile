...
# Build ninja-build from source.
# ```bash
WORKDIR /var/tmp/build/ninja-build
RUN curl -fsSL https://github.com/ninja-build/ninja/archive/refs/tags/v1.11.1.tar.gz | \
    tar -xzf - --strip-components=1 && \
    cmake -Bbuild-cmake && \
...
# Installs the ccache from source.
#     https://github.com/ccache/ccache/blob/master/doc/INSTALL.md
WORKDIR /var/tmp/build/ccache
RUN curl -fsSL https://github.com/ccache/ccache/releases/download/v4.8.2/ccache-4.8.2-linux-x86_64.tar.xz | \
...
RUN yum makecache && yum install -y c-ares-devel
...
# https://github.com/lvc/abi-dumper/pull/29. We can switch back to `yum install
