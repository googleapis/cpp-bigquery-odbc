...
readonly CPP_BIGQUERY_ODBC_DRIVER_MANAGER_SETUP_CURR_DIR=$(pwd)
...
if [ $(gsutil ls gs://${GCS_BUCKET}/odbc | grep -c odbc-driver.zip) -eq 0 ] ; \
...
if [ $(stat -c%s /opt/odbc-driver/connection/key.json) -lt 100 ] ; \
...
if [ $(ls /opt/odbc-driver/ | grep -c googlebigqueryodbc) -eq 0 ] ; \
