...
StatusOr<CancelJobResponse> DefaultBigQueryJobRestStub::CancelJob(
  ...
  rest_internal::RestContext context;
  auto start = absl::Now();
  auto res = rest_stub_->Post(context, std::move(*rest_request), {empty_span});
  std::cout << "Debug :: curl time tken "
            << absl::FormatDuration(absl::Now() - start) << std::endl;
  // Call the rest stub and parse the RestResponse.
  return ParseFromRestResponse<CancelJobResponse>(std::move(res));
}
...
StatusOr<QueryResponse> DefaultBigQueryJobRestStub::Query(
  ...
  // 5) Call the rest stub and parse the RestResponse.
  rest_internal::RestContext context;
  auto payload = filtered_json.dump();
  auto body = absl::MakeConstSpan(payload);
  auto start = absl::Now();
  auto res = rest_stub_->Post(context, std::move(*rest_request), {body});
  std::cout << "Debug :: [Query] [Post] Time Taken "
            << absl::FormatDuration(absl::Now() - start) << std::endl;

  auto rest_res = ParseFromRestResponse<QueryResponse>(std::move(res));
  return rest_res;
